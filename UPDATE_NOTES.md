...
- Ordens extras de DCA anteriormente não eram marcadas como tal na criação, resultando em disparos duplos; agora ambas as rotas (SHORT e LONG) chamam `createOrder(..., isDCAOrder=true)` garantindo simetria e eliminando duplicações.
- Lógica de entrada BUY foi generalizada para suportar estratégia LONG: o fluxo agora valida `tradeSide === 'BUY'` independentemente de `strategy` e calcula a variação/condições apropriadas para fechar SHORT ou abrir LONG, mantendo as mesmas salvaguardas de tendência e filtros de 24h. Para LONG a condição de entrada considera queda (dip buy) e não subida. Isso corrige o comportamento onde bots LONG nunca iniciavam negociações. Além disso, o log “Aguardando condições” passou a ser suprimido durante a fase de entrada de LONG quando não há posição aberta (evitando mensagens desnecessárias); o aviso continua ativo para fechamento de SHORT e para trades LONG após a compra.
- DCA agora registra o preço de disparo (triggerPrice) e utiliza-o para calcular o espaçamento mínimo entre ordens extras. O valor armazenado em `lastActionPrice` é ajustado para o extremo mais desfavorável (maior para SHORT, menor para LONG), garantindo que ossextras subsequentes não ocorram em distâncias menores que `targetPercent` devido a slippage de mercado. Logs de depuração (`[DCA] triggerContra` e avisos de gap) facilitam auditoria de disparos. Esta correção aplica‑se simetricamente a estratégias LONG e SHORT.
